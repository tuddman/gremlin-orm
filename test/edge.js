...
describe('Edge Model', () => {
  beforeEach(done => {
    g.queryRaw('g.V().drop()', () => {
      Person.create(
        {name: 'John', age: 18, dob: '12/18/1999', developer: true},
        (err, result) => {
          john = result;
          Person.create(
            {name: 'Bob', age: 23, dob: '04/30/1994', developer: false},
            (err, result) => {
              bob = result;
              done();
            },
  ...
  describe('Create', () => {
    it('Should throw error if no callback is passed', () => {
      assert.throw(
        () => {
          Knows.create(john, bob, {duration: 5});
        },
        Error,
  ...
  describe('Find', () => {
    it('Should find and return the first edge with matching properties', done => {
      Person.create(
        {name: 'Sam', age: 38, dob: '12/18/1979', developer: true},
        (err, result) => {
          sam = result;
    ...
    it('Should find and return all vertices connected by relevant edges with matching properties', done => {
      Person.create(
        ...
        (err, result) => {
          sam = result;
          Person.create(
            {name: 'Steven', age: 10, dob: '05/11/2007', developer: false},
            (err, result) => {
              steven = result;
              Person.create(
                {name: 'Susan', age: 17, dob: '09/30/2000', developer: true},
                (err, result) => {
                  susan = result;
                  Person.create(
                    {
