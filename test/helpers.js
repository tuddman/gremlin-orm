...
describe('Helpers', () => {
  ...
  describe('familiarizeAndPrototype', () => {
    it('Should return array with vertex methods', done => {
      g.queryRaw(
        `g.addV('person').property('name','John').property('age', 24)`,
        (err, result) => {
    ...
    it('Should return array with edge methods', done => {
      g.queryRaw(
        ...
        (err, result) => {
          ...
          g.queryRaw(
            `g.addV('person').property('name','Jane').property('age', 24)`,
            (err, result) => {
              let janeID = result[0].id;
              g.queryRaw(
                `g.V(${johnID}).addE('knows').property('duration', 1).to(g.V(${janeID}))`,
                (err, result) => {
    ...
    it('Should return Gremlin string chainable object if no callback present', done => {
      g.queryRaw(
        ...
        (err, result) => {
          ...
          expect(obj.gremlinStr).to.equal(
      ...
      Person.create({name: 'John', age: 20}, (err, result) => {
        ...
        Person.create({name: 'Jane', age: 20}, (err, result) => {
          ...
          Person.findAll({}, (err, results) => {
            expect(results.getGremlinStr()).to.equal(
              `g.V("${results.map(el => el.id).join('","')}")`,
            );
            done();
          });
        });
  ...
  describe('dateGetMillis', () => {
    ...
    it('Should convert a Date object to integer time', () => {
      expect(typeof Person.dateGetMillis(new Date('12/18/1999'))).to.equal(
        'number',
      );
    });
  ...
  describe('checkSchema', () => {
    ...
    it('Should check required properties if checkRequired is true', () => {
      ...
      expect(obj[Object.keys(obj)[0]][0]).to.equal(
        `A valid value for 'name' is required`,
      );
    });
