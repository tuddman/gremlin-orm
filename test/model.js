...
describe('Model', () => {
  beforeEach(done => {
    g.queryRaw('g.V().drop()', () => {
      g.queryRaw(
        "g.addV('person').property('name','Victoria').property('age', 12)",
        () => {
          g.queryRaw(
            "g.addV('person').property('name','Samantha').property('age', 47)",
            () => {
              let knowsProps =
                ".property('duration', 1).property('how', 'golf days')";
              knowsProps +=
                ".property('anniversary', 1458101167000).property('duration', 1)";
              g.queryRaw(
                `g.V().has('name','Victoria').addE('knows')${knowsProps}.to(g.V().has('name','Samantha'))`,
                () => {
                  done();
                },
              );
  ...
  describe('Query', () => {
    ...
    it('Should run a query on the database', done => {
      Person.query(
        "g.V().hasLabel('person').has('name', 'Victoria')",
        (err, result) => {
  ...
  describe('Update', () => {
    ...
    it('Should throw error if no callback is passed', () => {
      assert.throw(
        () => {
          Person.find({name: 'John'}).update({name: 'Tommy'});
        },
        Error,
    ...
    it('Should not update vertex if date type prop is wrong', done => {
      Person.findAll({name: 'Victoria'}).update(
        {dob: 'date'},
        (err, result) => {
          expect(err).to.not.equal(null);
          done();
    ...
    it('Should update properties on the database for Edge Model', done => {
      g.queryRaw(
        "g.addV('person').property('name','Peter').property('age', 22)",
        () => {
          g.queryRaw(
            "g.V().has('name','Victoria').addE('knows').property('duration', 2).to(g.V().has('name','Peter'))",
            () => {
    ...
    it('Should not update edge if date type prop is wrong', done => {
      Knows.findAll({duration: 1}).update(
        {anniversary: 'date'},
        (err, result) => {
          expect(err).to.not.equal(null);
          done();
  ...
  describe('Order', () => {
    beforeEach(done => {
      g.queryRaw(
        "g.addV('person').property('name','Bobby').property('age', 22)",
        () => {
          g.queryRaw(
            "g.addV('person').property('name','Bobby').property('age', 33)",
            () => {
              g.queryRaw(
                "g.addV('person').property('name','Harry').property('age', 28)",
                () => {
                  let knowsProps = ".property('how', 'cooking class')";
                  knowsProps +=
                    ".property('anniversary', 1458901167000).property('duration', 1)";
                  let bobby33 = "g.V().has('name','Bobby').has('age', 33)";
                  g.queryRaw(
                    `${bobby33}.addE('knows')${knowsProps}.to(g.V().has('name','Harry'))`,
                    () => {
                      done();
                    },
                  );
          ...
          g.queryRaw(
            ...
            () => {
              g.queryRaw(
                ...
                () => {
                  let knowsProps =
                    ".property('duration', 1).property('how', 'cooking class')";
                  knowsProps += ".property('anniversary', 1458901167000)";
                  let bobby = "g.V().has('name','Bobby')";
                  g.queryRaw(
                    `${bobby}.addE('knows')${knowsProps}.to(g.V().has('name','Harry'))`,
                    () => {
                      done();
                    },
                  );
