...
describe('gremlin-orm', () => {
  ...
  describe('defineVertex', () => {
    ...
    it('Should define a new vertex model', () => {
      const Person = g.defineVertex('person', {
    ...
    it('Should create schema with required flag', () => {
      ...
      expect(Person.schema[Object.keys(Person.schema)[0]].required).to.equal(
        true,
      );
    });
    it('Should create schema with string type', () => {
      ...
      expect(Object.keys(Person.schema)[0]).to.equal('name');
      expect(Person.schema[Object.keys(Person.schema)[0]].type).to.equal(
        'string',
      );
    });
  ...
  describe('defineEdge', () => {
    ...
    it('Should create a new vertex model with schema', () => {
      const Knows = g.defineEdge('knows', {
        how: {type: g.STRING},
        friends: {type: g.BOOLEAN},
      });
      expect(Object.keys(Knows.schema)).to.have.lengthOf(2);
    });
    it('Should create schema with required flag', () => {
      const Knows = g.defineEdge('knows', {
        duration: {type: g.NUMBER, required: true},
      });
      expect(Object.keys(Knows.schema)[0]).to.equal('duration');
      expect(Knows.schema[Object.keys(Knows.schema)[0]].required).to.equal(
        true,
      );
    });
    it('Should create schema with string type', () => {
      ...
      expect(Knows.schema[Object.keys(Knows.schema)[0]].type).to.equal(
        'string',
      );
    });
    it('Should create schema with number type', () => {
